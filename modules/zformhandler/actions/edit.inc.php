<?php
    ...
    $form = ZFormHandler::getForm('edit',$rowId);
    $rowModel = ZFactory::createModel('zform_handler');

    if (isset($formData['zformhandler_edit_submit'])) {
        if ($form->isValid($formData)) {
            //fill an empty row with form data
            $updateData = ZFormHandler::hydrateRowData($formData);
            //update row
            $res = 0;
            if ($rowId >0) {
